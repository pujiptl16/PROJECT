...
<%
	...
	else
	{
	    if(request.getParameter("ApplyLeave") != null) {
	        ...
	        PreparedStatement ps = con.prepareStatement("INSERT INTO tblleave (User_id, Leave_Reason, Leave_From, Leave_To) VALUES (?, ?, ?, ?)");
	        ps.setString(1, aid);
	    ...
	    if(request.getParameter("edit") != null) {
	        ...
	        PreparedStatement ps = con.prepareStatement("SELECT * FROM tblleave WHERE Leave_id = ?");
	        ps.setString(1, id);
	        ResultSet rs = ps.executeQuery();

	        if(rs.next()) {
	            status = rs.getString("Leave_Status");
	            remark = rs.getString("Leave_Remarks");
	        }
	    }
	    ...
	    if(request.getParameter("UpdateLeave") != null) {
	        ...
	        PreparedStatement ps = con.prepareStatement("UPDATE tblleave SET Leave_Status = ?, Leave_Remarks = ?, Leave_ApprovedBy = ? WHERE Leave_id = ?");
                ...
                <div class="row">
                    <div class="col-12">
                        ...
                        <h5 class="card-subtitle"> Manage Leave</h5>
                                <h6 class="card-title mt-5"><i class="me-1 font-18 mdi mdi-numeric-1-box-multiple-outline"></i><a href="starter-kit.jsp?apply">Apply Leave</a></h6>
                           <% if(request.getParameter("apply")!=null){ %>
                            ...
                            <form action="" method="post" class="form-horizontal mt-4">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="datetime-local" name="FDate" class="form-control" value="" placeholder="From Date" required>
                                </div>
                                <div class="form-group">
                                    <label>To Date</label>
                                    <input type="datetime-local" name="TDate" class="form-control" value="" placeholder="To Date" required>
                                </div>
                                <div class="form-group">
                                    <label>Reason</label>
                                    <input type="text" name="Reason" class="form-control" value="" placeholder="Please Specify Reason in Maximum 75 character." required>
                                </div>
                                <div class="form-group">
                                        <div class="col-sm-12">
                            ...
                            <form action="" method="post" class="form-horizontal mt-4">
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="Status" class="form-control" required>
                                        ...
                                        <option value="Granted" <% if(status.equals("Granted")){ %> selected <% } %> >Granted</option>
                                ...
                                <div class="comment-widgets" style="height:500px;">
                                   ...
                                   <%
                                        ...
                                        <%
											...
											String query = "SELECT c.FirstName, c.LastName, c.EnrollmentNo, a.Leave_id, a.Leave_From, a.Leave_To, a.Leave_Reason, a.Leave_Status, a.Leave_Remarks, a.Leave_ApprovedBy " +
											               "FROM tblleave a " +
											...
											%>
											    <tr>
											        <td colspan="9"><center>No leaves applied yet.</center></td>
											    </tr>
											<%
											} else {
											    int num = 1;
											...
											%>
											        <tr>
											            ...
											            <td><%= rs.getString("Leave_Remarks") == null ? "" : rs.getString("Leave_Remarks") %></td>
											            <% if (!rs.getString("Leave_Status").equals("Pending")) { %>
											                <td colspan="2"></td>
											            <% } else { %>
											                <td><a href="starter-kit.jsp?edit=<%= rs.getString("Leave_id") %>">Edit</a></td>
											            <% } %>
											        </tr>
											    <% num++; } while (rs.next());
											}
