...
else
{
    ...
    if (isset($_GET['edit'])) {
        ...
        if($_GET['ReCType']=='Request'){
            $record = mysqli_query($db, "SELECT * FROM tblrequest WHERE Req_id=$id");
            if ($record->num_rows == 1 ) {
                $n = mysqli_fetch_array($record);
                $status=$n['Req_Status'];
                $remark=$n['Req_Remarks'];
            }
        }else{
            $record = mysqli_query($db, "SELECT * FROM tblcomplain WHERE Cmp_id=$id");
            if ($record->num_rows == 1 ) {
                $n = mysqli_fetch_array($record);
                $status=$n['Cmp_Status'];
                $remark=$n['Cmp_Remarks'];
            }
        }
    ...
    if (isset($_POST['UpdateRec'])) {
        ...
        if($GET['ReCType']=='Complain'){
            mysqli_query($db, "UPDATE `tblcomplain` SET `Cmp_Remarks`='$remark',`Cmp_Status`='$status',`Cmp_ClosedBy`='$aid',`Cmp_UpdatedTS`='$currentDate' WHERE `Cmp_id`='$id'");
        }else{
            mysqli_query($db, "UPDATE `tblrequest` SET `Req_Remarks`='$remark',`Req_Status`='$status',`Req_UpdatedBy`='$aid',`Req_UpdatedTS`='$currentDate' WHERE `Req_id`='$id'");
        }
                            ...
                            <form action="" method="post" class="form-horizontal mt-4">
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="Status" class="form-control" required>
                                        ...
                                        <option value="Acknowleged" <?php if($status=="Acknowleged"){ ?> selected <?php }?>>Acknowleged</option>
                            ...
                            <div class="table-responsive">
                                <div class="comment-widgets" style="height:500px;">
                                    ...
                                    <?php if (isset($_SESSION['message'])): ?>
                                        ...
                                        <?php 
                                            $id=$_SESSION['userid'];
                                            $record = mysqli_query($db, "SELECT * FROM tblrequest a 
                                        ...
                                        <?php    
                                            if($record->num_rows==0){
                                                ?>
                                                <tr>
                                                    <td colspan=8><center>No any ReCs added yet.</center></td>
                                                </tr>
                                                <?php
                                            } else {
                                            $num=1;
                                             ...
                                             ?>
                                                ...
                                                <th scope="row"><?php echo $num; $num=$num+1; ?></th>
                                                    ...
                                                    <?PHP if($row['Req_Status'] != "Pending") { ?>
                                                        <td colspan=2></td>
                                                    <?php } else { ?>
                                                    <td><a href="CRC.php?edit=<?php echo $row['Req_id']; ?>&ReCType='Request'">Edit</a></td>
                                                    <td><a href="CRC.php?del=<?php echo $row['Req_id']; ?>&ReCType='Request'" onclick="return checkDelete()">Delete</a></td>
                                                    <?php } ?>
                                                </tr>
                                                <?php } }
                                                $record = mysqli_query($db, "SELECT * FROM tblcomplain a 
                                                LEFT JOIN tblhostel_record b on b.user_id=a.user_id
                                                LEFT JOIN tblregister c on b.user_id=c.user_id
                                                WHERE b.Hostel_id=$hid;");
                                                    ...
                                                    ?>
                                                        ...
                                                        <th scope="row"><?php echo $num; $num=$num+1; ?></th>
                                                            ...
                                                            <?PHP if($row['Cmp_Status'] != "Pending") { ?>
                                                                <td colspan=2></td>
                                                            <?php } else { ?>
                                                            <td><a href="CRC.php?edit=<?php echo $row['Cmp_id']; ?>&ReCType='Complain'">Edit</a></td>
                                                            <td><a href="CRC.php?del=<?php echo $row['Cmp_id']; ?>&ReCType='Complain'" onclick="return checkDelete()">Delete</a></td>
                                                            <?php } ?>
                                                        </tr>
                                                        <?php 
                                                    } 
